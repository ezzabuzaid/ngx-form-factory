...
export class FieldModelerComponent {
  ...
  constructor(@Inject(ARGS) public args: any) {
    ...
    this.addLine('readonly', this.args.readonly);
    this.addLine('value', this.args.value);
    this.addLine(
      'type',
      this.args.type ? `EFieldType.${EFieldType[this.args.type]}` : '',
      true
    );
    ...
    this.addLine(
      'max',
      this.args.maxFilter && `new Date(${this.args.maxFilter})`,
      true
    );
    this.addLine(
      'min',
      this.args.minFilter && `new Date(${this.args.minFilter})`,
      true
    );
  ...
  private decorateErrors(errors: Record<string, any>) {
    ...
    if (new Object(errors) !== errors) return;
...
export function pickAsObject<T>(object: T, prop: keyof T) {
