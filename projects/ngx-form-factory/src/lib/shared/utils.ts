...
export function assertNotNullOrUndefined<T>(value: T, debubLabel?: string): asserts value is NonNullable<T> {
    if (value === null || value === undefined) {
        throw new Error(`${!!debubLabel ? debubLabel : ''} cannot be undefined or null.`);
    }
}
